...
function simple_hotel_crm_render_bookings_page() {
    ...
    if ( empty( $rows ) ) {
        ...
        foreach ( $rows as $row ) {
            ...
            echo '<td>' . esc_html( (string) $row['room_count'] ) . '<br /><span class="description">' . esc_html( (string) $row['room_codes'] ) . '</span></td>';
...
function simple_hotel_crm_render_booking_detail_page() {
    ...
    echo '</select></label><label style="flex:1 1 50%;min-width:180px;">' . esc_html__( 'Channel', 'simple-hotel-crm' ) . '<br><select name="source_channel">';
